    ...
    int minMoves(vector<string>& classroom, int energy) {
        ...
        vector<vector<vector<vector<int>>>> dist(
            R, vector<vector<vector<int>>>(
                   C, vector<vector<int>>(energy + 1,
                                          vector<int>(1 << num_litter, -1))));
                ...
                if (nr >= 0 && nr < R && nc >= 0 && nc < C &&
                    ...
                    if (current_e > 0) {
                        ...
                        if (destination_cell_char == 'L') {
                            int litter_idx =
                                litter_coord_to_idx_map.at({nr, nc});
                        ...
                        if (dist[nr][nc][energy_at_new_cell]
                                [new_mask_after_move] == -1 ||
                            next_total_moves < dist[nr][nc][energy_at_new_cell]
                                                   [new_mask_after_move]) {
                            dist[nr][nc][energy_at_new_cell]
                                [new_mask_after_move] = next_total_moves;
                            q.push({nr, nc, energy_at_new_cell,
                                    new_mask_after_move});
                        }
                    }
                }
