...
public:
    vector<int> majorityElement(vector<int>& nums) {
        // We are looking for all elements that appear more than ⌊n/3⌋ times in the array.
        // According to the extended Boyer-Moore Voting Algorithm, 
        ...
        for (int i = 0; i < size; i++) {
            if (count1 == 0 && nums[i] != candidate2) {
                ...
                // Current number doesn't match any candidate, decrease both counts
                count1--;
                count2--;
            }
        }
