class Solution {
public:
    bool isConnected(unordered_map<int, unordered_set<int>>& mp, int k, int i,
                     int j) {
        for (auto t : mp[i])
            k -= (mp[j].find(t) != mp[j].end()) ? 1 : 0;
        return (k <= 0);
    }
    ...
    int add(int a, int b, vector<int>& par) {
        ...
        par[pb] = pa;
        return (pa != pb) ? 1 : 0; // return 1 if bothe are in different group
                                   // before ese return 0
    }
        ...
        for (int i = 0; i < p.size(); ++i) {
            for (int j = i + 1; j < p.size(); ++j) {
                if (isConnected(mp, k, i, j))
                    count -= add(
                        i, j,
                        par); // check connected or not, if connected then add
